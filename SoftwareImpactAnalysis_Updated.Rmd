---
title: "Software Impact Data Analysis: Updated"
subtitle: "General trends and exploratory stats"
author: "Awan"
date: "`r Sys.Date()`"
output: 
  html_document:
...
## REDEFINE total mentions using only (Allusion+Deposition+Usage)
```{r}
...
## (NEW) Stratification by: How to cite instruction (WITHOUT BioConductor and other-R packages)
...
## (NEW) Overlap of tools between survey data and Tool Analysis
...
d1 <- regdat %>% 
  ...
  labs(x = "Release Year", y = "log(total Usage)", subtitle = "Scatterplot of log-Usage vs Release Year") +
...
d11 <- regdat %>% 
  ...
  geom_point(aes(shape = socialMedia)) + 
  geom_smooth(method = "lm", se = T) +
  labs(x = "Release Year", y = "log(total totalMentions)", subtitle = "Scatterplot of log-totalMentions vs Release Year") +
  theme_light() +coord_equal()
...
# healthmetrics
d34 <- regdat %>% 
  ggplot(aes(x = releaseYear, y = log(totalMentions), col = ownDOI)) + 
  geom_point(aes(shape = ownDOI)) + 
  geom_smooth(method = "lm", se = T) +
  labs(x = "Release Year", y = "log(total totalMentions)", subtitle = "Scatterplot of log-totalMentions vs Release Year") +
  theme_light() +coord_equal()
